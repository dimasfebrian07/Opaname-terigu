...
function renderWA(data){
 if(!data.length){$("waPreview").textContent="Belum ada data opname.";return}
 let g=grouped(data),d=new Date(data[0].date+"T00:00:00");
 let dateText=d.toLocaleDateString("id-ID",{weekday:"long",day:"numeric",month:"long",year:"numeric"});
 let lines=["Stock terigu & tapioka Gwip stage 1",`Shift ${$("filterShift").value||data[0].shift}`,dateText,""];
 [["Terigu Soft","Stock Terigu soft"],["Terigu Medium","Stock terigu medium"],["Tapioka","Stock Tapioka :"]].forEach(([key,title],i)=>{
   lines.push(title,"");
   Object.entries(g[key]).forEach(([brand,kg])=>lines.push(`${brand} : ${fmtKg(kg)}`));
   if(i<2)lines.push("",""); 
 });
